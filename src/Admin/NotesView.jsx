...
const TreeTableDemo = () => {
  ...
  const onRowClick = (event) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_KEY}/document/getHtmlContains/${event.node.secId}/${sessionStorage.getItem('email')}`
      )
      .then((res) => {
    ...
    return (
      <div>
        <div style={{ display: "flex" }} >
          {serialNumber}
          <div title={rowData.notes} className="spanStyleClassNotes">) {rowData.notes}</div>
        </div>
      </div>
    );
  };
