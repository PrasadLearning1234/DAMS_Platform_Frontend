...
const DocumentSearch = () => {
  ...
  const [searching, setSearcing] = useState("");
  ...
  const onClick = (name, position) => {
    ...
    if (position) {
      ...
      axios
        .get(
          `${process.env.REACT_APP_API_KEY}/document/getHtmlContains/${position.id}/${emailId}`
        )
        .then((res) => {
  ...
  const customBody = (rowData, column) => {
    return (
      <span
        ...
        dangerouslySetInnerHTML={{ __html: rowData[column.field] }}
      />
    );
  };
  ...
  return (
    <div>
      ...
      <Card style={{ height: "32rem", paddingBottom: "2rem" }}>
        <Toast ref={toast} />
        ...
        <Tag style={{ backgroundColor: "#D2D7E2",color:"#2D2D2D"}} className="mr-2" > {item.Suggestion}</Tag>
            ...
            >
              ...
              <span
                ...
                // dangerouslySetInnerHTML={{ __html: docView.replace(new RegExp(searching, 'g'), `<span class="highlighted">${searching}</span>`) }}
              />
